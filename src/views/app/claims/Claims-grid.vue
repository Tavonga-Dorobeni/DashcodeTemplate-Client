...
const claims = computed(() => currentUser.value.roles.includes('PATIENT') ? store.getters.allClaims.filter(c => appointments.value.map(a => a.AppointmentID).includes(c.AppointmentID)) : store.getters.allClaims);
