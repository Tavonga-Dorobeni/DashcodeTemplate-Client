<template>
  <div class="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
    <Card bodyClass="p-6" v-for="(item, i) in procedures" :key="i">
      <!-- header -->
...
const actions = ref([
  ...
  },
  {
    ...
    doit: (data) => {
      data.procedure_roles = data.roles.map(r => r.name)
      view.value = false;
      currentProcedure.value = data;
      modal2.value.openModal();
    },
  },
  {
    ...
    doit: (data) => {
      ...
      store.dispatch('deleteProcedure', data)
      .then(response => {
...
const updateProcedure = () => {
  const toast = useToast();
  store.dispatch('updateProcedure', currentProcedure.value)
  .then(response => {
